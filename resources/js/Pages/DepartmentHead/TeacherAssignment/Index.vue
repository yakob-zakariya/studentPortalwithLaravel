<template>
  <div>
    ...
    <h1 class="mb-8 text-3xl font-bold">
      ...
      <span class="font-medium text-indigo-400">/</span> Choose Semster to Assign Teacher
    </h1>
    <div class="max-w-4xl overflow-hidden bg-white rounded-md shadow">
      <form @submit.prevent="store">
        <div class="flex flex-wrap p-8 -mb-6 -mr-6">
          ...
          <!-- Select Batch -->
          <select-input v-model="form.batch_id" @change="fetchSections" :error="form.errors.batch_id" class="w-full pb-8 pr-6 lg:w-1/2" label="Select Batch">
            <option :value="null" />
            <option v-for="batch in batches" :key="batch.id" :value="batch.id">{{ batch.name }} ({{ batch.department.code }})</option>
          </select-input>

           <!-- Select section -->
           <select-input v-model="form.section_id" :error="form.errors.section_id" class="w-full pb-8 pr-6 lg:w-1/2" label="Select section">
            <option :value="null" />
    ...
    async fetchSections() {
      if (this.form.batch_id) {
        try {
          ...
          this.sections = response.data; // Update sections with data from API
          this.form.section_id = null; // Reset selected section when batch changes
        } catch (error) {
          console.error('Error fetching semesters:', error);
        }
      } else {
